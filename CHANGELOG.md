# 25.2-qStudio (BedWars1058-qStudio, by qSa3ed)

BedWars1058-qStudio is a fork of BedWars1058 that keeps the original
gameplay intact and adds a configurable enhancement layer on top.

## Modern item compatibility layer
- new reusable ModernItemCompatibility architecture: modern gameplay
  concepts are represented on legacy versions through configurable
  legacy materials (display item, behaviour, name, lore, effects)
- Mace: configurable smash weapon (legacy representation: iron shovel)
  with falling-attack damage scaling, area knockback, cooldown,
  particles and sounds
- Wind Charge: snowball based projectile with self-launch, vertical and
  horizontal knockback, configurable strength and cooldown
- Firework Boost, Dash (multi-use items with charges stored on the item),
  Grappling Hook, Temporary Bridge (auto-expiring team blocks),
  Bridge Builder, Teleport Beacon (activation delay, enemy interruption,
  limited uses), Emergency Teleport (channelling, damage/move cancel),
  Base Shield (team damage reduction) and EMP Utility Pulse
- all items are gated per arena group and can be purchased through the
  regular shop configuration

## Combat
- configurable knockback profiles per arena group (ground, air, sprint,
  projectile, explosion multipliers)
- hit feedback: optional sounds, action bar messages and attack cooldown
- combat tagging with configurable duration and blocked commands
- projectile tuning for arrows, snowballs and eggs
- explosive tuning: fireball and TNT radius, TNT jump damage reduction

## Traps
- qStudio trap actions: blindness, slowness, weakness, alarm, knockback,
  marking and exhaustion traps with configurable effects, plus
  QSTrapTriggerEvent integration in the base enter flow

## Team upgrades
- new upgrade actions: forge tiers, trap capacity, regen aura,
  defense aura, anti-invisibility and temporary generator boost

## Shop
- purchase limits per life and per game with cooldowns
- modern items purchasable through the existing shop format

## Progression
- quest system (daily, weekly, once) with configurable rewards,
  progress action bar and a quest GUI
- achievements evaluated from persistent statistics
- quest state persisted through the remote database (async)

## Administration & API
- /qstudio admin command: diagnostics, config validation, debug toggle
- config validator for materials, slots and shop references
- generator status overview (admin diagnostics + API)
- public qstudio API package: QStudioApi, ModernItems and new events
  (QSTrapTriggerEvent, QSPlayerPurchaseEvent, QSItemUseEvent,
  QSQuestCompleteEvent, QSAchievementUnlockEvent)
- extra PlaceholderAPI identifiers: qstudio_level, qstudio_quests_done,
  qstudio_achievements, qstudio_arena, qstudio_team

## Build
- vipfeatures-api pinned to 1.1 (CodeMC) so the project builds without
  the dead andrei1058 repository
- sidebar library resolved from CodeMC snapshots
- PlaceholderAPI resolved through repo.helpch.at
