...
public class PAPISupport extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String s) {
        ...
        /* qStudio placeholders */
        if (s.startsWith("qstudio_")) {
            com.andrei1058.bedwars.api.qstudio.QStudioApi qstudio = com.andrei1058.bedwars.api.qstudio.QStudio.get();
            if (qstudio == null) return "";
            switch (s.replaceFirst("qstudio_", "")) {
                case "level":
                    response = String.valueOf(BedWars.getLevelSupport().getPlayerLevel(player));
                    break;
                case "xp":
                ...
                case "forge": {
                    ...
                    response = currentArena == null ? "0" :
                            String.valueOf(qstudio.getForgeLevel(currentArena.getTeam(player)));
                    break;
                }
                case "current_team": {
                    IArena currentArena = Arena.getArenaByPlayer(player);
                    response = currentArena == null || currentArena.getTeam(player) == null ? "NONE"
                            : currentArena.getTeam(player).getName();
                    break;
                }
                case "quests_done":
                    response = String.valueOf(qstudio.getCompletedQuests(player.getUniqueId()).size());
                    break;
                case "achievements":
                    response = String.valueOf(qstudio.getUnlockedAchievements(player.getUniqueId()).size());
                    break;
            }
        }
        return response;
