...
public class SQLite implements Database {
    ...
    public void init() {
        ...
        try {
            ...
            try (Statement st = connection.createStatement()) {
                sql = "CREATE TABLE IF NOT EXISTS qstudio_progress (uuid VARCHAR(36) PRIMARY KEY, " +
                        "quest_progress TEXT, quest_completed TEXT, achievements TEXT);";
                st.executeUpdate(sql);
            }
    ...
    @Override
    public void saveQuestData(UUID uuid, String questData, String questCompleted) {
        try {
            checkConnection();
            String sql = "INSERT OR REPLACE INTO qstudio_progress (uuid, quest_progress, quest_completed, achievements) " +
                    "SELECT ?, ?, ?, COALESCE((SELECT achievements FROM qstudio_progress WHERE uuid = ?), '');";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setString(1, uuid.toString());
                ps.setString(2, questData);
                ps.setString(3, questCompleted);
    ...
    @Override
    public String getQuestProgress(UUID uuid) {
        return queryColumn(uuid, "quest_progress");
    }

    @Override
    public String getQuestCompletions(UUID uuid) {
        return queryColumn(uuid, "quest_completed");
    }

    @Override
    public void saveAchievements(UUID uuid, String serialized) {
        try {
            checkConnection();
            String sql = "INSERT OR REPLACE INTO qstudio_progress (uuid, quest_progress, quest_completed, achievements) " +
                    "SELECT ?, COALESCE((SELECT quest_progress FROM qstudio_progress WHERE uuid = ?), ''), " +
                    "COALESCE((SELECT quest_completed FROM qstudio_progress WHERE uuid = ?), ''), ?;";
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setString(1, uuid.toString());
    ...
    @Override
    public String getAchievements(UUID uuid) {
        return queryColumn(uuid, "achievements");
    }

    private String queryColumn(UUID uuid, String column) {
        try {
            checkConnection();
            try (PreparedStatement ps = connection.prepareStatement(
                    "SELECT " + column + " FROM qstudio_progress WHERE uuid = ?;")) {
                ps.setString(1, uuid.toString());
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
