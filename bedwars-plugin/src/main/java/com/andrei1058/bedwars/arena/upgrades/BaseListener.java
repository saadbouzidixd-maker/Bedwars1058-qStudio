        ...
        if (team.isMember(e.getPlayer())) {
            ...
            if (!team.getActiveTraps().isEmpty()) {
                if (!team.isBedDestroyed()) {
                    com.andrei1058.bedwars.api.upgrades.EnemyBaseEnterTrap trap = team.getActiveTraps().get(0);
                    com.andrei1058.bedwars.api.qstudio.events.QSTrapTriggerEvent trapEvent =
                            new com.andrei1058.bedwars.api.qstudio.events.QSTrapTriggerEvent(team, e.getPlayer(),
                                    trap.getNameMsgPath().replace("base-trap-", ""));
                    Bukkit.getPluginManager().callEvent(trapEvent);
                    if (!trapEvent.isCancelled()) {
                        trap.trigger(team, e.getPlayer());
                    }
