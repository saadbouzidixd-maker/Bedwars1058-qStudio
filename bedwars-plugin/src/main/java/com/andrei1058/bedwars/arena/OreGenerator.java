...
public class OreGenerator implements IGenerator {
    ...
    public void spawn() {
        ...
        // qStudio: team forge level and boosts accelerate the generator
        int effectiveDelay = delay;
        int effectiveAmount = amount;
        if (bwt != null) {
            double speed = com.andrei1058.bedwars.qstudio.upgrades.GeneratorBoostManager.getSpeedMultiplier(bwt);
            if (speed > 1.0) {
                effectiveDelay = Math.max(1, (int) Math.round(delay / speed));
            }
            effectiveAmount += com.andrei1058.bedwars.qstudio.upgrades.GeneratorBoostManager.getForgeBonusAmount(bwt);
        }

        if (lastSpawn == 0) {
            lastSpawn = effectiveDelay;
            ...
            if (plugin.getConfig().getBoolean(ConfigPath.GENERAL_CONFIGURATION_ENABLE_GEN_SPLIT)) {
                ...
                for (Object o : players) {
                    ...
                    item.setAmount(effectiveAmount);
    ...
    private void dropItem(Location location, int amount) {
        // qStudio: team forge bonus scales the amount of the team generator
        int boostedAmount = amount;
        if (bwt != null && amount == this.amount) {
            boostedAmount = amount + com.andrei1058.bedwars.qstudio.upgrades.GeneratorBoostManager.getForgeBonusAmount(bwt);
        }
        for (int temp = boostedAmount; temp > 0; temp--) {
