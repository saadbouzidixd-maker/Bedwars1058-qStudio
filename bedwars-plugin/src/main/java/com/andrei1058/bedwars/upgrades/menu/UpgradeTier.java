        ...
        for (String action : UpgradesManager.getConfiguration().getYml().getStringList(parentName + "." + name + ".receive")) {
            ...
            switch (type[0].trim().toLowerCase()) {
                ...
                case "command":
                    ...
                    break;
                case "forge":
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.ForgeAction();
                    break;
                case "trap-capacity":
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.TrapCapacityAction();
                    break;
                case "regen-aura":
                    int regenAmp = data.length >= 1 ? parseInt(data[0], 0) : 0;
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.RegenAuraAction(regenAmp);
                    break;
                case "defense-aura":
                    int defAmp = data.length >= 1 ? parseInt(data[0], 0) : 0;
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.DefenseAuraAction(defAmp);
                    break;
                case "anti-invisibility":
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.AntiInvisibilityAction();
                    break;
                case "gen-boost":
                    int seconds = data.length >= 1 ? parseInt(data[0], 30) : 30;
                    ua = new com.andrei1058.bedwars.qstudio.upgrades.QSUpgradeActions.GenBoostAction(seconds);
                    break;
            }
